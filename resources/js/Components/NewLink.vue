...
const resetFormData = () => {
	form.name = "";
	form.partner_email_service = "";
	form.failure_page_url = "";
	form.success_page_url = "";
	form.campaign = "";
	form.zapier_webhook_url = "";
};
...
const newLinkForm_submit = async () => {
	...
	if (props.embedded) {
		URL = route("embedMarketing_link.store");
	} else {
		URL = route("marketing_link.store");
	}
	...
	await axios
		.post(URL, {
			...
			user_unique_public_id: props.user_unique_public_id,
		})
						...
						<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
							...
							<div class="mt-2">
								<p class="text-sm text-gray-500">
									Begin your journey. Save the link generated here and also give
									it to your partner to send from their email service.
								</p>
							</div>
						</div>
					</div>
