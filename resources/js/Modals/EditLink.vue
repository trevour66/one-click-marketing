...
const editLinkForm_submit = async () => {
	let URL = route("marketing_link.update", {
		marketing_link_id: form.email_marketing_links_id,
	});
	...
	await axios
		.patch(URL, {
			...
			link_identifier: form.link_identifier,
		})
		.then(function (response) {
			const data = response?.data?.data ?? false;

			form.email_marketing_platforms_id = data.email_marketing_platforms_id;
			form.failure_page_url = data.failure_page_url;
			form.full_link = data.full_link;
			form.link_identifier = data.link_identifier;
			form.name = data.name;
			form.success_page_url = data.success_page_url;

			form.campaign = data.campaign;
			form.zapier_webhook_url = data.zapier_webhook_url;

			Swal.fire({
				icon: "success",
				title: "Your link has been updated!",
				html: `<div class="p-4 text-sm text-gray-800 rounded-lg bg-gray-50 text-center">
					<span class="font-medium">${form.full_link}</span>
					</div>`,
				footer:
...
onMounted(() => {
	...
	form.campaign = props.linkData.campaign;
	form.zapier_webhook_url = props.linkData.zapier_webhook_url;
});
</script>
						...
						<div class="sm:flex sm:items-start">
							<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
								<h3
									...
									id="modal-title"
								>
									Update your Marketing link
								</h3>
								<div class="mt-2">
									<p class="text-sm text-gray-500">
										Save the link generated here and also give it to your
										partner to send from their email service.
									</p>
								</div>
							</div>
						</div>
